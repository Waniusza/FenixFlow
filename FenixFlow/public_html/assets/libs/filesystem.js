...
fileSystem.factory('fileSystem', ['$q', '$timeout', function ($q, $timeout) {
        ...
        var fileSystem = {
            ...
            getFolderContents: function (path) {
                ...
                console.log("1 getFolderContents");
                path = path.replace(/^\//, "");

                var def = $q.defer();

                console.log("2 getFolderContents");
                function getContent(rootDir, folders) {
                    rootDir.getDirectory(folders[0], {}, function (dirEntry) {

                        console.log("4 getFolderContents", folders.length);
                        var dirReader = dirEntry.createReader();
                        if (folders.length > 1) {
                            getContent(dirEntry, folders.slice(1));
                        } else {
                            console.log("5 getFolderContents", folders.length);
                            dirReader.readEntries(function (entries) {
                                console.log("6 getFolderContents", entries);
                                safeResolve(def, entries);
                                console.log("7 getFolderContents", entries);
                            }, function (e) {
                                console.log("3err getFolderContents");
                                safeReject(def, {text: "Error reading entries", obj: e});
                            });
                        }
                    }, function (e) {

                        console.log("2er getFolderContents");
                        safeReject(def, {text: "Error getting directory", obj: e});
                    });
                }

                fsDefer.promise.then(function (fs) {
                    console.log("3 getFolderContents", fs.root, path.split('/'));
                    getContent(fs.root, path.split('/'));
                }, function (err) {

                    console.log("1er getFolderContents");
                    def.reject(err);
                });
